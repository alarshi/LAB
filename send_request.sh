...
do
  ...
  cat $f >> post.json
  echo '''"},"ToRecipients": [{"EmailAddress": {"Name": " ","Address": "breq_fast@iris.washington.edu"}}]}} ''' >> post.json
  curl -X POST --data-binary "@post.json" https://outlook.office365.com/api/v1.0/me/sendmail --header "Content-Type:application/json" --insecure --verbose --user "asaxena@memphis.edu:Ivegotthis02"
